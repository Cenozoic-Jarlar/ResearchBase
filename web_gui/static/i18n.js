...
const I18N = {
  zh: {
    ...
    "research.flow": "静态流程",
    "research.profiles": "价值观框架（勾选自动匹配=不干预；勾具体框架=指定风格）",
    "research.tier": "模型档位（留空=自动：按 Agent 声明档位，未声明者按难度映射）",
    "research.timeout": "LLM 超时（秒，留空=用 .env 默认 90）",
    "research.autoArchive": "📥 跑完自动归档要点到资料库",
    "research.autoArchiveHint": "勾上后跑完会额外调一次 LLM 提炼要点摘要写入资料库（全文不入库）；不勾=只输出文章不入库",
    "research.start": "▶ 开始任务",
    "research.cancel": "⏹ 取消",
    "import.topic": "目标话题（入库去向：AI 提炼 / 选现有话题 / 输入新话题）",
    "import.newTopic": "＋ 新建话题库",
    "import.newTopicHint": "建空话题目录，以后再往里加资料",
  ...
  en: {
    ...
    "research.profiles": "Value frameworks (check auto-match=off; check specific=override)",
    ...
    "research.autoArchive": "📥 Auto-archive key points to library after run",
    "research.autoArchiveHint": "Triggers one extra LLM call to distill a summary (full text not stored); unchecked=just output article",
    "research.start": "▶ Start task",
    "research.cancel": "⏹ Cancel",
    "import.topic": "Target topic (AI extract / pick existing / type new)",
    "import.newTopic": "＋ New topic library",
    "import.newTopicHint": "Create empty topic folder, add materials later",
